...
export default function Account() {
  ...
  useEffect(() => {
    async function fetchBalance() {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/Account/balance`, authHeaders());
    ...
    async function fetchProfileIfNeeded() {
      if (!signinUser || !signinUser.email) {
        try {
          const r = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/user/me`, authHeaders());
          const user = r.data;
          if (user) {
            setSigninUser(user);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/v1/user/Update`, body, authHeaders());
  ...
  const handleDeleteAccount = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/delete-account`, authHeaders());
      handleLogout();
    } catch (err) {
